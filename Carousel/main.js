jQuery(document).ready(function($){
    ...
    function sliderSize(current){
            ...
            if(current){
                ...
                activeSlide = index;
                $(dots[index - nos]).addClass('active')
            }
        ...
        //Set a variable to decide scroll direction, -1 for backward, 1 for forward
             ...
             if(direction === -1){
                $('.slider-dot').addClass('disabled');
                 ...
                 setTimeout(function(){
                     ...
                     $('.slider-dot').removeClass('disabled');
                     $('.slider-dot').removeClass('active');
                     $(dots[activeSlide - nos]).addClass('active');
                 },300);  
             }
         ...
         //Slider Dot function
         const theDot = document.querySelectorAll('.slider-dot');
         for(let i = 0; i<theDot.length; i++){
            $(theDot[i]).click(function(){
                var divideWidth = $('.item').outerWidth(); //Get Item width, Use outerWidth() to include the 'padding' size
                let destination = i + nos; 
                const item = document.querySelectorAll('.item');
                var moveSlide = divideWidth * destination;
                activeSlide = destination;

                $('.slider-dot').addClass('disabled');
                $('.slide-btn').addClass('disabled'); 
                $('.slider').css('transition','all 0.3s ease-out');
                $('.slider').css('transform','translateX(-'+moveSlide+'px)');
